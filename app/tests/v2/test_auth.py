...
class TestAuthentication(base):
    ...
    def test_user_login(self):
        """Testing Logging In."""
        post_response = self.client.post('/api/v2/auth/register', data=json.dumps(self.registration_payload), content_type=self.content_type)
        ...
        response = self.client.post('/api/v2/auth/login', data=json.dumps(self.login_payload), content_type=self.content_type)
        response_data = json.loads(response.data.decode())
    ...
    def test_logging_in_non_existent_user(self):
        ...
        self.assertEqual(post_response.status_code, 201)
        self.assertEqual(post_response_data["message"], "User registered successfully. Please Log in.")
        response = self.client.post('/api/v2/auth/login', data=json.dumps(self.bad_login_payload), content_type=self.content_type)
        response_data = json.loads(response.data.decode())
